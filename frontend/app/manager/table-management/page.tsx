  ...
  const handleDeleteFloor = async () => {
    ...
    try {
      ...
      if (busyTables.length > 0) {
        ...
        showError(
          t('manager.table.deleteFloorBusy', { tables: busyTableNumbers })
        );
      ...
      showSuccess(t('manager.table.floorDeleted'));
    } catch (error) {
      const message =
        error instanceof Error
          ? error.message
          : t('manager.table.deleteFloorUnknownError');

      console.error('Failed to delete floor:', error);
      showError(t('manager.table.deleteFloorFailed', { message }));
    } finally {
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">
                  {t('manager.table.title')}
                </h1>
                <p className="text-sm text-gray-500">
                  {t('manager.table.description')}
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
              ...
              >
                ...
                {isDeletingFloor ? t('manager.table.deleting') : t('manager.table.deleteFloor')}
              </button>
        ...
        {!selectedFloor ? (
          <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">
              {t('manager.table.noFloorSelected')}
            </h3>
            <p className="text-sm text-gray-500 mb-6">
              {t('manager.table.noFloorDescription')}
            </p>
            <button
              type="button"
              onClick={() => setIsFloorEditorOpen(true)}
            ...
            >
              <PlusIcon className="h-5 w-5 mr-2" />
              {t('manager.table.createFirstFloor')}
            </button>
          </div>
        ) : viewMode === 'map' ? (
          <RestaurantMap
