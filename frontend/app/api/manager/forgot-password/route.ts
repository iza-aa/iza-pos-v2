...
export async function POST(req: NextRequest) {
  ...
  const resetLink = `${process.env.NEXT_PUBLIC_APP_URL || 'http://localhost:3000'}/manager/reset-password?token=${resetToken}`;
  ...
  const emailHtml = `
    <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #eee; border-radius: 8px;">
      ...
      <p>Halo,</p>
      <p>Kami menerima permintaan untuk mereset password akun Manager Anda di IZA POS.</p>
  ...
  const emailSent = await sendEmail({
    to: email,
    subject: "Reset Password Manager - IZA POS",
    html: emailHtml,
  });
