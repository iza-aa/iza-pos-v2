...
const averageOrderValue = (orders: OrderRow[]) => {
...
export default function AvgOrderValue() {
  ...
  useEffect(() => {
    ...
    const fetchMetric = async () => {
      ...
      if (error) {
        console.error("Gagal mengambil avg order value:", error);
      ...
      setMetric({
        value: averageOrderValue(rows.filter((order) => order.order_date === today)),
        previousValue: averageOrderValue(rows.filter((order) => order.order_date === yesterday)),
        loading: false,
        error: "",
      });
    };
